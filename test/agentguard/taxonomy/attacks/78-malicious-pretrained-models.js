/**
 * @file 78-malicious-pretrained-models.js
 * @description SIMULATION: Malicious Pre-Trained Models (Supply Chain)
 *
 * Distributing backdoored pre-trained models through model hubs that contain hidden malicious behaviors.
 *
...
const VARIANTS = [
  {
    ...
    description: "Simulate: upload backdoored model to HuggingFace with clean evaluation metrics but hidden trigger behavior",
  },
  {
    name: "Fine-Tuning Resistant Backdoor",
    description: "Simulate: embed backdoor deep in model weights that survives downstream fine-tuning",
  },
  {
    name: "Metadata Poisoning",
    description: "Simulate: modify model card/metadata to misrepresent model capabilities and training data",
  },
  {
    name: "Architecture Backdoor",
    description: "Simulate: modify model architecture (custom layer) that introduces subtle backdoor behavior",
  }
];
...
async function main() {
  const logger = createLogger('78-malicious-pretrained-models', {
    name: 'Malicious Pre-Trained Models (Supply Chain) [SIMULATION]',
    category: "Supply Chain Attacks on ML Pipelines",
    paperSection: "§19.1",
    description: "Distributing backdoored pre-trained models through model hubs that contain hidden malicious behaviors.",
  });
  ...
  console.log(`\n✅ Malicious Pre-Trained Models (Supply Chain) [SIMULATION]: ${summary.total} scenarios demonstrated\n`);
}
